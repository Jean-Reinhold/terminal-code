...
Rust color/theme/profile tests plus production CLI/helper integration cover generation, extension/registry/live files, cleanup, idempotence, embedded platform-correct JetBrains Mono install, palette `--theme` output, JSONC `--theme <file>` import with exact errors/live application, raw terminal color validation/fallbacks, full-theme helper output, preload capture, and Rust multi-socket success/refusal/stale-peer fan-out. The generated Electron adapter invokes only the Rust helper—no duplicate JavaScript theme logic. C10 remains draft until an actual terminal theme-change event is driven through terminal-browser.
