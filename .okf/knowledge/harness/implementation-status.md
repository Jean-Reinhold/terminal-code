...
* Production Rust `tode` binary with help/version, compatibility/open parsing, extensions, import/theme/upgrade-check/uninstall, existing-window reuse, profile/CSS, daemon, browser launch, and shutdown.
...
* `tode-harness catalog check`: 22 contracts, 10 scenarios, 119 mapped legacy tests, and 103 contract-mapped Rust tests.
...
* One hundred seventeen Rust workspace tests passed:
...
* Remaining shortcut/timing/skill commands, full upgraded-output transaction integration, and direct dual-target differential execution.
